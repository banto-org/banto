...
21 skill / 6 agent / 49 hook / 7 rule。MCP 同梱なし。セキュリティレビュー・コードレビューは
...
その後 **/reload-plugins（または Claude Code 再起動）**で日本語セットがロードされる。選択は永続化され、**プラグイン更新をまたいで保持**されるので、**以降この操作は不要**（英語へ戻すときだけ `/set-language en` ＋ /reload-plugins）。
