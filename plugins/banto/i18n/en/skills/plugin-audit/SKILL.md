...
> - **plugin-audit (this skill)** = **quality** audit of a plugin / single skill (official compliance + 14-axis structural evaluation). Pass `skills/<name>` as the argument for a **per-skill audit (= skill-audit)** — provided as a mode of this skill rather than a separate skill.
...
### Phase 9: 14-axis quality audit

Phases 1-8.5 are official compliance checks for "does it work". Phase 9 is the **14-axis quality audit**: static structural axes (computed by the audit scripts) plus judgment axes (run via independent subagents — Reviewer = Fresh Agent).

Details:
- Evaluation criteria (14-axis definitions): [`references/scoring.md`](references/scoring.md)
...
**The 14 axes** (static = computed by the audit scripts; agent = judged by independent subagents):
...
| 11 | Usage (commits + .ai-context mentions over N days → active/mentioned/dormant/likely-trim) | static (`usage`) |
...
**Static axes — run the scripts** (Axes 1 / 2 / 3 / 7 / 9 / 10 / 11 / 12 / 14, plus detection material for 5 / 6):
...
- `plugin-audit-usage.sh`: classifies usage from the past N days of git log + .ai-context mentions (Axis 11)
...
- `plugin-audit-assets.sh`: walks each skill's subtree (`references/` + nested) and reports inventory / unnecessary files / orphans / duplicates / Axis 14 hygiene (the Axis 14 + Axis 2 extension that fills the gap where collect.sh only sees SKILL.md; details in scoring.md Axis 14)
