...
The naming rule and the archive procedure follow SKILL.md "Auto-archive when all tasks are complete" + `references/auto-archive.md`. `/ai-context phase-done` (`references/phase-done.md`) performs it with verification included.
...
(Follows the naming rule of `references/auto-archive.md`. The hook may detect full completion and guide this procedure.)

Note: the search layer is auto-regenerated by the hook (no manual step).

## Related

- Running the next task is `/ai-context next` (`references/next.md`)
