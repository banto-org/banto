...
Match it to the document's **selected theme**. Add this at the top of each .mmd (the example below is navy; copy `primaryColor` = the theme's wash and `primaryBorderColor` = the hairline from color-themes.md):

```
%%{init: {'theme':'neutral','themeVariables':{
  'fontFamily':'Hiragino Sans, Noto Sans JP, sans-serif',
  'primaryColor':'#e8eef5','primaryBorderColor':'#c5d4e3',
  'primaryTextColor':'#1d232e','lineColor':'#4a5568'
}}}%%
```
...
- Background is already set to transparent (blends into a wash-background box)

### 3. Inline embedding

Paste the SVG file's contents (`<svg …>…</svg>`) straight into the template's figure slot:
...
- The template already implements `figure svg { max-width: 100%; height: auto; }`
  ...
  <diagram name="Architecture" id="d1">
    <mxGraphModel dx="800" dy="600" grid="1" gridSize="10" page="1" pageWidth="827" pageHeight="1169">
      <root>
        ...
        <mxCell id="n1" value="Web Server" style="rounded=1;whiteSpace=wrap;fillColor=#e8eef5;strokeColor=#c5d4e3;" vertex="1" parent="1">
          <mxGeometry x="120" y="120" width="160" height="60" as="geometry"/>
        </mxCell>
        <mxCell id="n2" value="DB" style="shape=cylinder3;whiteSpace=wrap;fillColor=#e8eef5;strokeColor=#c5d4e3;" vertex="1" parent="1">
          <mxGeometry x="400" y="110" width="100" height="80" as="geometry"/>
        </mxCell>
        <mxCell id="e1" style="edgeStyle=orthogonalEdgeStyle;rounded=1;strokeColor=#4a5568;" edge="1" parent="1" source="n1" target="n2">
...
Match colors to the selected theme's tokens (fill = wash / stroke = hairline / lines `#4a5568` / accent color only for emphasis — see color-themes.md; the example above is navy).
