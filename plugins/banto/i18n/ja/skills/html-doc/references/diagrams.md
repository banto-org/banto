...
資料の**選択テーマと揃える**。各 .mmd の先頭に（下例は navy。`primaryColor`=テーマの wash、`primaryBorderColor`=hairline を color-themes.md から転記）:
...
SVGファイルの中身（`<svg …>…</svg>`）を雛形の figure スロットへそのまま貼る:
...
- 雛形側に `figure svg { max-width: 100%; height: auto; }` 実装済み
  ...
  <diagram name="構成図" id="d1">
    <mxGraphModel dx="800" dy="600" grid="1" gridSize="10" page="1" pageWidth="827" pageHeight="1169">
      <root>
        ...
        <mxCell id="n1" value="Webサーバ" style="rounded=1;whiteSpace=wrap;fillColor=#e8eef5;strokeColor=#c5d4e3;" vertex="1" parent="1">
...
色は選択テーマのトークン（fill=wash / stroke=hairline / 線 `#4a5568` / 強調のみ accent — color-themes.md 参照。上例は navy）に揃える。
