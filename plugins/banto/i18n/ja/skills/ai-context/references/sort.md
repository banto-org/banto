...
| 呼び出し | モード | 対象 |
|---|---|---|
| `/ai-context sort` | **内部モード**（既定） | `.ai-context/` 内の誤配置振り分け |
...
### 目的
`.ai-context/` 内の誤配置を**対話形式で振り分け**する（doctor で検出したもの）。ユーザー承認なしに移動しない。
...
プロジェクト内に散在するドキュメントを対話形式で整理する。`.ai-context/` の外（ルート直置き / `docs/` 等）が対象。
