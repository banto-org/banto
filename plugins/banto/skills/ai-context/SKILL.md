...
3. combined.txt is auto-regenerated by the PostToolUse hook on a Write to docs
...
`combined.txt`, the grep target for search, is **auto-regenerated by a hook (`ai-context-combined-rebuild.sh`) on save**:
- on a write to `{base}/decisions/`
- on a write to `{base}/docs/`
