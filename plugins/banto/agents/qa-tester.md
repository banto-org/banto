...
1. `mcp__claude-in-chrome__tabs_context_mcp` → always call first to get the connected browsers and tab list (without it, no tab is reachable)
2. `mcp__claude-in-chrome__tabs_create_mcp` → create a new tab for testing (never reuse existing tabs)
3. `mcp__claude-in-chrome__navigate` → navigate to URL
4. `mcp__claude-in-chrome__read_page` → check page content
5. `mcp__claude-in-chrome__computer` / `form_input` / `find` → click, type, and take screenshots
